import { useParams } from "react-router-dom";
import { FaGithub } from "react-icons/fa";
import { FiExternalLink } from "react-icons/fi";
import projects from "../data/projects.js";
import { useEffect } from "react";
import { HashLink } from "react-router-hash-link";

const ProjectDetail = () => {
  const { id } = useParams();
  const project = projects.find((p) => p.id === parseInt(id));

  useEffect(() => {
    window.scrollTo(0, 0);
  }, [id]);

  if (!project) {
    return (
      <main
        className="min-h-screen flex flex-col items-center justify-center bg-gray-900 text-gray-100"
        role="main"
      >
        <h2 className="text-3xl font-bold mb-4">Project not found</h2>
        <HashLink
          to="/#projects"
          className="text-indigo-500 hover:underline focus:outline-none focus:ring-2 focus:ring-indigo-400 rounded"
        >
          Back to Projects
        </HashLink>
      </main>
    );
  }

  return (
    <main
      className="min-h-screen bg-gray-900 text-gray-100 pt-32 px-8 md:px-16 lg:px-24"
      role="main"
      aria-labelledby="project-name"
    >
      <h1
        id="project-name"
        className="text-4xl font-bold mb-6 text-indigo-300 scroll-mt-32"
      >
        {project.name}
      </h1>

      <img
        src={project.image}
        alt={`Screenshot of ${project.name} project`}
        className="w-full rounded-lg mb-6 object-cover"
        loading="lazy"
      />

      <p className="mb-4">
        <strong>Type:</strong> {project.type}
      </p>

      {/* Technologies Used */}
      {project.technologies && project.technologies.length > 0 && (
        <section aria-labelledby="tech-used" className="mb-6">
          <h2
            id="tech-used"
            className="text-2xl font-semibold mb-2 text-indigo-300"
          >
            Technologies Used
          </h2>
          <ul className="flex flex-wrap gap-2" role="list">
            {project.technologies.map((tech, i) => (
              <li
                key={i}
                className="bg-indigo-600 text-white px-3 py-1 rounded-full text-sm"
              >
                {tech}
              </li>
            ))}
          </ul>
        </section>
      )}

      {/* What I Did */}
      {project.whatIDid && project.whatIDid.length > 0 && (
        <section aria-labelledby="what-i-did" className="mb-6">
          <h2
            id="what-i-did"
            className="text-2xl font-semibold mb-2 text-indigo-300"
          >
            What I Did
          </h2>
          <ul
            className="list-disc list-inside space-y-1 text-gray-300"
            role="list"
          >
            {project.whatIDid.map((item, i) => (
              <li key={i}>{item}</li>
            ))}
          </ul>
        </section>
      )}

      {/* Problem & Solution */}
      <section aria-labelledby="problem-solution" className="mb-6">
        <h2 id="problem-solution" className="sr-only">
          Problem and Solution
        </h2>
        <p className="mb-4">
          <strong>Problem:</strong> {project.problem}
        </p>
        <p className="mb-4">
          <strong>Solution:</strong> {project.solution}
        </p>
      </section>

      {/* Links */}
      <div className="flex gap-4 mt-6" role="region" aria-label="Project links">
        <a
          href={project.github}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`View ${project.name} project on GitHub`}
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-full shadow-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-400"
        >
          <FaGithub aria-hidden="true" /> GitHub
        </a>
        <a
          href={project.liveDemo}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`View live demo of ${project.name}`}
          className="flex items-center gap-2 bg-sky-600 hover:bg-sky-500 text-white px-4 py-2 rounded-full shadow-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-400"
        >
          <FiExternalLink aria-hidden="true" /> Live Demo
        </a>
      </div>

      <HashLink
        to="/#projects"
        className="mt-8 inline-block text-indigo-500 hover:underline focus:outline-none focus:ring-2 focus:ring-indigo-400 rounded"
      >
        ← Back to Projects
      </HashLink>
    </main>
  );
};

export default ProjectDetail;
